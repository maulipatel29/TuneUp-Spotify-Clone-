...
router.get(
  ...
  async (req, res) => {
    ...
    const playlists = await Playlist.find({ owner: artistId }).populate(
      "owner"
    ); // find all id whose owner was artistid.
    return res.status(200).json({ data: playlists });
  }
);
...
router.get(
  ...
  async (req, res) => {
    ...
    const playlists = await Playlist.find({ owner: artistId }); // find all id whose owner was artistid.
    return res.status(200).json({ data: playlists });
  }
);
...
router.post(
  ...
  async (req, res) => {
    ...
    await playlist.save(); // .save() is use for locally store the data in database.
